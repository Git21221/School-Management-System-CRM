...
export function DashboardWidget({ widget, data }: Props) {
  ...
  if ((widget.type === "bar" || widget.type === "area") && widget.dataSource === "fee-trend") {
    const Chart = widget.type === "area" ? AreaChart : BarChart;
    return (
      <Card className="p-5 h-full">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-foreground">{widget.title}</h3>
  ...
  if (widget.type === "pie" && widget.dataSource === "course-enrollment") {
        ...
        {pieData.length === 0 ? (
          ...
          <>
            <ResponsiveContainer width="100%" height={150}>
              <PieChart>
                <Pie isAnimationActive={false} data={pieData} cx="50%" cy="50%" innerRadius={42} outerRadius={65} paddingAngle={3} dataKey="value">
                  {pieData.map((entry) => (
