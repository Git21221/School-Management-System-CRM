...
async function runSqlFile(connection: mysql.Connection, filename: string) {
...
async function provisionDatabase(rootConn: mysql.Connection, config: SetupConfig) {
  ...
  await rootConn.query(
    `CREATE USER IF NOT EXISTS ${userIdent}@'localhost' IDENTIFIED BY ?`,
    [config.dbPassword]
  );
  await rootConn.query(`ALTER USER ${userIdent}@'localhost' IDENTIFIED BY ?`, [config.dbPassword]);
  await rootConn.query(
    `GRANT SELECT, INSERT, UPDATE, DELETE ON ${dbId}.* TO ${userIdent}@'localhost'`
  );
  await rootConn.query('FLUSH PRIVILEGES');
}

async function seedInstituteAndAdmin(connection: mysql.Connection, config: SetupConfig) {
  const adminId = randomUUID();
  const passwordHash = await bcrypt.hash(config.adminPassword, 12);

  console.log('▶ Creating institute profile and admin account...');
  await connection.query(
    `INSERT INTO institute_settings (id, name, email, phone, academic_year)
     VALUES (1, ?, ?, ?, ?)
...
function writeEnv(config: SetupConfig) {
...
async function main() {
  ...
  let rootConn: mysql.Connection | undefined;
  let appConn: mysql.Connection | undefined;
