...
export function instituteToDatabaseName(name: string): string {
  const slug = name
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '_')
...
export function defaultDbUser(databaseName: string): string {
  const base = databaseName.slice(0, 28);
  return `${base}_app`;
}

/** Default admin email when none is provided. */
export function defaultAdminEmail(databaseName: string): string {
