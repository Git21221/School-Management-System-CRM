 ...
 * Usage: npx tsx src/scripts/smoke-test-apis.ts
 */
...
function pass(status: number, expected: number | number[]): boolean {
...
async function main() {
  ...
  for (const t of tests) {
    ...
    results.push({
      ...
      note: r.status !== 200 ? String((r.json.error as { message?: string })?.message ?? '') : undefined,
    });
  }
