...
export const v = {
  ...
  addressOptional: () =>
    z.preprocess(
      emptyToNull,
      z.string().trim().regex(PATTERNS.address, 'Address must be at least 5 characters').nullable().optional()
    ),

  gradeOptional: () =>
    z.preprocess(
      emptyToNull,
      z.string().regex(PATTERNS.grade, 'Grade must be A+, A, B+, B, C, F, or -').nullable().optional()
    ),
