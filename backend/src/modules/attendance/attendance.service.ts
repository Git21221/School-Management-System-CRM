...
export async function upsertAttendance(data: BulkUpsertAttendanceInput, markedBy: string) {
  ...
  try {
    for (const record of data.records) {
      ...
      await connection.query(
        `INSERT INTO attendance_records (id, student_id, batch_id, record_date, status, marked_by)
         VALUES (?, ?, ?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE status = VALUES(status), marked_by = VALUES(marked_by)`,
        [recordId, record.studentId, data.batchId, data.date, record.status, markedBy]
      );
    }
...
export async function listAttendanceRecords(months = 12) {
  ...
  const [rows] = await pool.query(
    `SELECT ar.student_id AS studentId,
     ...
     ORDER BY ar.record_date DESC, ar.student_id ASC`,
    [safeMonths]
  );
  return rows as {
    studentId: string;
...
export async function getAttendanceReport(batchId: string, month: string) {
  ...
  (summaryRows as any[]).forEach((row) => {
    ...
    else if (row.status === 'leave') summary.leave = row.count;
  });

  return {
    summary,
    students: studentRows.map((row: any) => ({
