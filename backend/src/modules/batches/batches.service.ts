...
import { CreateBatchInput, UpdateBatchInput } from './batches.schema.js';
...
export async function getBatchStudents(batchId: string) {
  const [rows] = await pool.query(
    'SELECT id, name, phone, email, status, photo_url FROM students WHERE batch_id = ? ORDER BY name ASC',
    [batchId]
  );
  return rows as any[];
}
...
export async function createBatch(data: CreateBatchInput) {
  ...
  if (data.facultyId) {
    const [faculties] = await pool.query('SELECT id FROM faculty WHERE id = ?', [data.facultyId]) as any[];
  ...
  await pool.query(
    `INSERT INTO batches (id, course_id, name, timing, faculty_id, status, start_date, end_date)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      data.id,
      data.courseId,
...
export async function deleteBatch(id: string) {
  ...
  const [students] = await pool.query('SELECT COUNT(*) as count FROM students WHERE batch_id = ?', [id]) as any[];
