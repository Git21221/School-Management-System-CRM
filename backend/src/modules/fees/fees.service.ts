...
export async function getPaymentByReceipt(receipt: string) {
  const [rows] = await pool.query(`
    SELECT p.receipt, p.amount, p.mode, p.pay_date as payDate, p.remarks, p.created_at as createdAt,
...
export async function collectFee(data: CollectPaymentInput, createdBy: string) {
  ...
  try {
    ...
    const [students] = await connection.query(
      'SELECT id, name, fees_total, fees_paid FROM students WHERE id = ? AND status != \'Deleted\' FOR UPDATE',
      [data.studentId]
    ) as any[];
    ...
    while (!isUnique && attempts < 5) {
      ...
      const [existing] = await connection.query(
        'SELECT receipt FROM payments WHERE receipt = ?',
        [receiptId]
      ) as any[];
    ...
    await connection.query(
      `INSERT INTO payments (receipt, student_id, amount, mode, pay_date, remarks, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [
        receiptId,
        data.studentId,
