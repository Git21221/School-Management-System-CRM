...
export async function getSettings() {
  ...
  if (!settings) {
    ...
    await pool.query(
      `INSERT INTO institute_settings (id, name, email, phone) 
       VALUES (1, 'TechAcademy CRM', 'admin@techacademy.com', '+91 98450 10001')`
    );
    const [freshRows] = await pool.query('SELECT * FROM institute_settings WHERE id = 1') as any[];
...
export async function updateInstituteSettings(data: UpdateInstituteInput) {
...
export async function updateReceiptSettings(data: UpdateReceiptConfigInput) {
  ...
  await pool.query(
    'UPDATE institute_settings SET receipt_config = ? WHERE id = 1',
...
export async function updateCertificateSettings(data: UpdateCertificateConfigInput) {
  const current = await getSettings();
  ...
  await pool.query(
    'UPDATE institute_settings SET certificate_config = ? WHERE id = 1',
    [JSON.stringify(mergedConfig)]
  );
