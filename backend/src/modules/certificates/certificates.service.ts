...
export async function getCertificateByNo(certNo: string) {
  const [rows] = await pool.query(`
    SELECT cert.cert_no as certNo, cert.grade, cert.issue_date as issueDate, cert.authorised_by as authorisedBy, cert.created_at as createdAt,
...
export async function createCertificate(data: IssueCertificateInput) {
  ...
  const [students] = await pool.query('SELECT id FROM students WHERE id = ? AND status != \'Deleted\'', [data.studentId]) as any[];
  ...
  while (!isUnique && attempts < 5) {
    ...
    const [existing] = await pool.query('SELECT cert_no FROM certificates WHERE cert_no = ?', [certNo]) as any[];
  ...
  await pool.query(
    `INSERT INTO certificates (cert_no, student_id, course_id, grade, issue_date, authorised_by)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [certNo, data.studentId, data.courseId, data.grade, data.issueDate, data.authorisedBy]
  );
