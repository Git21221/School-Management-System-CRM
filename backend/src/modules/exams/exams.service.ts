...
export async function createExam(data: CreateExamInput) {
  ...
  const [courses] = await pool.query('SELECT id FROM courses WHERE id = ?', [data.courseId]) as any[];
  if (courses.length === 0) {
    throw new NotFoundError(`Course with ID "${data.courseId}" not found`);
  }

  const [batches] = await pool.query('SELECT id FROM batches WHERE id = ?', [data.batchId]) as any[];
  if (batches.length === 0) {
    throw new NotFoundError(`Batch with ID "${data.batchId}" not found`);
  }

  await pool.query(
    `INSERT INTO exams (id, title, course_id, batch_id, exam_date, max_marks, status)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [data.id, data.title, data.courseId, data.batchId, data.examDate, data.maxMarks, data.status]
  );
...
export async function getExamMarksList(examId: string) {
...
export async function saveExamMarks(examId: string, data: SaveMarksInput) {
  const exam = await getExamById(examId);
  if (!exam) {
    throw new NotFoundError(`Exam with ID "${examId}" not found`);
  }
  ...
  for (const record of data.marks) {
    if (record.marks > exam.maxMarks) {
      throw new ValidationError(
        `Marks for student "${record.studentId}" cannot exceed maximum exam marks (${exam.maxMarks})`
      );
    }
  }
  ...
  try {
    for (const record of data.marks) {
      ...
      await connection.query(
        `INSERT INTO exam_marks (id, exam_id, student_id, marks)
         VALUES (?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE marks = VALUES(marks)`,
        [markId, examId, record.studentId, record.marks]
      );
    }
...
export async function getExamResultsDetails(examId: string) {
  const exam = await getExamById(examId);
  if (!exam) {
    throw new NotFoundError(`Exam with ID "${examId}" not found`);
  }
  ...
  const [marks] = await pool.query(
    `SELECT s.id as studentId, s.name as studentName, em.marks
     FROM exam_marks em
     JOIN students s ON em.student_id = s.id
     WHERE em.exam_id = ? AND s.status != 'Deleted'
     ORDER BY em.marks DESC`,
    [examId]
  ) as any[];
  ...
  marks.forEach((row: any) => {
    if (row.marks > highest) highest = row.marks;
