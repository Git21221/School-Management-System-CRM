...
export async function getKPIs() {
  const [[studentsRow]] = await pool.query(
    "SELECT COUNT(*) as count FROM students WHERE status != 'Deleted'"
  ) as any[];
  const [[activeStudentsRow]] = await pool.query(
    "SELECT COUNT(*) as count FROM students WHERE status = 'Active'"
  ) as any[];
  const [[newAdmissionsRow]] = await pool.query(
    `SELECT COUNT(*) as count FROM students
     WHERE status != 'Deleted'
       AND YEAR(admission_date) = YEAR(CURDATE())
       AND MONTH(admission_date) = MONTH(CURDATE())`
  ) as any[];
  const [[feesDueRow]] = await pool.query(
    `SELECT COALESCE(SUM(GREATEST(0, fees_total - fees_paid)), 0) as total
     FROM students WHERE status != 'Deleted'`
  ) as any[];
  const [[feesCollectedRow]] = await pool.query(
    `SELECT COALESCE(SUM(fees_paid), 0) as total FROM students WHERE status != 'Deleted'`
  ) as any[];
  const [[studentsWithFeesDueRow]] = await pool.query(
    `SELECT COUNT(*) as count FROM students
     WHERE status != 'Deleted' AND fees_paid < fees_total`
  ) as any[];
  const [[totalCoursesRow]] = await pool.query('SELECT COUNT(*) as count FROM courses') as any[];
  const [[activeCoursesRow]] = await pool.query(
    "SELECT COUNT(*) as count FROM courses WHERE status = 'Active'"
  ) as any[];
  const [[upcomingBatchesRow]] = await pool.query(
    "SELECT COUNT(*) as count FROM batches WHERE status = 'Upcoming'"
  ) as any[];
  const [[ongoingBatchesRow]] = await pool.query(
    "SELECT COUNT(*) as count FROM batches WHERE status = 'Ongoing'"
  ) as any[];
  const [[facultyRow]] = await pool.query('SELECT COUNT(*) as count FROM faculty') as any[];
  const [[revenueRow]] = await pool.query('SELECT COALESCE(SUM(amount), 0) as total FROM payments') as any[];
...
export async function getFeeTrend() {
  ...
  const dueByMonth = new Map(
    (dueRows as { month: string; due: number }[]).map((r) => [r.month, parseFloat(String(r.due)) || 0])
  );
  const collectedByMonth = new Map(
    (collectedRows as { month: string; collected: number }[]).map((r) => [
      r.month,
