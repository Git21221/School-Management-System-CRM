...
export async function getStudentsList(filters: StudentFilters) {
  const page = filters.page || 1;
  const limit = filters.limit || 10;
  const offset = (page - 1) * limit;

  const whereClauses: string[] = ["s.status != 'Deleted'"];
  const params: any[] = [];
...
export async function createStudent(data: CreateStudentInput) {
  ...
  await pool.query(
    `INSERT INTO students (id, name, phone, email, course_id, batch_id, guardian, guardian_phone, address, admission_date, fees_total, fees_paid, status, dob, grade, photo_url)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      data.id,
      data.name,
...
export async function deleteStudentSoft(id: string) {
  const existing = await getStudentById(id);
  if (!existing) {
    throw new NotFoundError(`Student with ID "${id}" not found`);
  }
