...
export async function loginUser(
  ...
  const user = await queryOne<UserRow>(
    'SELECT id, name, email, password_hash, role, phone, photo_url FROM users WHERE email = ?',
    [email]
  );

  const recordLoginAttempt = async (succeeded: boolean) => {
    try {
      await pool.query(
        'INSERT INTO login_attempts (email, ip_address, succeeded) VALUES (?, ?, ?)',
        [email, ipAddress || 'unknown', succeeded ? 1 : 0]
      );
    } catch (err) {
  ...
  await pool.query(
    ...
    [tokenId, user.id, tokenHash, expiresAt, ipAddress, userAgent]
  );
...
export async function refreshSession(
  ...
  const tokenRecord = await queryOne<RefreshTokenRow>(
  ...
  await pool.query(
    'INSERT INTO refresh_tokens (id, user_id, token_hash, expires_at, ip_address, user_agent) VALUES (?, ?, ?, ?, ?, ?)',
    [newTokenId, user.id, newTokenHash, newExpiresAt, ipAddress, userAgent]
  );
...
export async function getUserById(userId: string) {
  return queryOne<UserProfileRow>(
    'SELECT id, name, email, role, phone, photo_url, created_at FROM users WHERE id = ?',
    [userId]
  );
}
